...
// src/app/features/supervisor/services/decision.service.ts
var DecisionService = class _DecisionService {
  http;
  ...
  createFinalDecision(decision) {
    ...
    return this.http.post(this.apiUrl, cleanDecision, {
      headers: this.getAuthHeaders()
    });
  }
  // ✅ التحقق من وجود نتيجة نهائية للمنتسب
  checkExistingDecision(fileNumber) {
    return this.http.get(`${this.apiUrl}/by-file/${fileNumber}`, {
      headers: this.getAuthHeaders()
    });
  }
  /** سجل تغيير النتيجة النهائية (كان مرفوض → صار مقبول) */
  getDecisionHistory(applicantFileNumber) {
    return this.http.get(`${this.apiUrl}/History?applicantFileNumber=${encodeURIComponent(applicantFileNumber)}`, { headers: this.getAuthHeaders() }).pipe(map((res) => res?.data ?? []));
  }
  /** جلب القرارات النهائية حسب أرقام الملفات (لعمود النتيجة النهائية في القائمة) */
  getDecisionsByFileNumbers(fileNumbers) {
    if (!fileNumbers.length) {
      return of([]);
    }
    const params = fileNumbers.map((fn) => `fileNumbers=${encodeURIComponent(fn)}`).join("&");
    return this.http.get(`${this.apiUrl}/ByFileNumbers?${params}`, { headers: this.getAuthHeaders() }).pipe(map((res) => res?.data?.items ?? []));
  }
  // ✅ تحديث القرار النهائي باستخدام PUT
  updateFinalDecision(decisionId, decision) {
    const cleanDecision = {
      ...
      supervisorLastModifiedAt: (/* @__PURE__ */ new Date()).toISOString()
    };
    if (decision.earClinicID) {
      cleanDecision.earClinicID = decision.earClinicID;
    }
    ...
    return this.http.put(`${this.apiUrl}/${decisionId}`, cleanDecision, {
      headers: this.getAuthHeaders()
    });
  }
  static \u0275fac = function DecisionService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _DecisionService)(\u0275\u0275inject(HttpClient));
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _DecisionService, factory: _DecisionService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(DecisionService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
//# sourceMappingURL=chunk-TJK6RMDX.js.map
